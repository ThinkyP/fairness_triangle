...
def generate_synthetic_data(plot_data=False, n_samples=2000, disc_factor=np.pi / 4.0, seed=0, marg_dens_px = False):
    """
    ...
    for i in range (0, len(X)):
        ...
        if r < p1: # the first cluster is the positive class
            Y_sen.append(1.0) # 1.0 means its female
        else:
            Y_sen.append(0.0) # 0.0 -> male
    ...
    if marg_dens_px:
        pX = 0.5 * nv1.pdf(X) + 0.5 * nv2.pdf(X)
        return X, Y, Y_sen, pX    
    return X, Y, Y_sen

def get_syn_data_custom(mean1, mean2, n_samples=2000, disc_factor=np.pi / 4.0, seed=0):
    """
    Generate synthetic data for binary classification with a sensitive attribute.
